...
fun PaymentsScreen(
    ...
    LaunchedEffect(tab, shopName, customerName, phone, refreshKey) {
        delay(300)
        loading = true
        error = null
        try {
            if (tab == 0) outstanding = api.outstanding(query) else payments = api.payments(query)
        } catch (e: Exception) {
            error = e.message
        } finally {
            loading = false
    ...
    androidx.compose.material3.Scaffold(
        ...
        Column(Modifier.fillMaxSize().padding(padding)) {
            PageHeader(title = "Payments", subtitle = if (tab == 0) "Outstanding balances" else "Payment history", onRefresh = { refreshKey++ })
            TabRow(selectedTabIndex = tab) {
                Tab(selected = tab == 0, onClick = { tab = 0 }, text = { Text("Outstanding") })
            ...
            when {
                ...
                tab == 1 && payments.isEmpty() -> EmptyState("No payments")
                tab == 0 -> LazyColumn {
                    ...
                    items(payments, key = { it.id }) { payment ->
                        GuruCard(onClick = { if (user.canEdit()) onEditPayment(payment.id) }) {
                            ...
                            if (user.canEdit() && payment.status == "pending") {
                                GuruOutlinedButton(
                                    text = "Mark complete",
                                    onClick = {
                                        scope.launch {
